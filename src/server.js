...
async function licver() {
  ...
  try {
    ...
    await hddserial.check(
      decrypt(key.toString().trim()).split("|")[0],
      async function (err, success, serial) {
        if (success || bypass) {
          ...
          var Resultado =
            "ERROR LICENCIA SOLO ES VALIDA AL HARDDISK:" +
            decrypt(key.toString().trim()).split("|")[0] +
            " - KEY: " +
            key.toString().trim();
...
function Apprun(licp) {
  if (licp.State == true) {
    ...
    app.get("/GetOnus", (req, res) => {
      ...
      fetch(
        "https://colnetwork.smartolt.com/api/onu/get_onus_statuses",
        requestOptions
      )
        ...
        .then((result) => {
          if (result.status == true) {
            result.response.forEach((OnusStatus) => {
              fetch(
                "https://colnetwork.smartolt.com/api/onu/get_onus_details_by_sn/" +
                  OnusStatus.sn,
                requestOptions
              )
                .then((response) => response.json())
    ...
    app.get("/EnabledOnunjdv/:id", async function (req, res) {
      ...
      fetch(
        `https://${app.get("Subdomain")}.smartolt.com/api/onu/enable/` +
          req.params.id,
        requestOptions
      )
        ...
        .then(async function (result2) {
          ...
          fetch(
            `https://${app.get(
              "Subdomain"
            )}.smartolt.com/api/onu/get_onus_details_by_sn/` + req.params.id,
            requestOptionsGet
          )
            .then((response) => response.json())
    ...
    app.get("/GetNameOnu/:id", function (req, res) {
      ...
      onusshe
        .find({ sn: req.params.id })
        .sort({ date: -1 })
        .then((empresas) =>
          res.json({
            status: true,
            sn: req.params.id,
            name: empresas[0].json.name,
          })
        )
    ...
    app.get("/DisableOnu/:id", async function (req, res) {
      ...
      fetch(
        `https://${app.get("Subdomain")}.smartolt.com/api/onu/disable/` +
          req.params.id,
        requestOptions
      )
        .then((response) => response.text())
