...
class TelnetTplinkUtilty {
    ...
    async onReady() {
        ...
        this.Currentlevel = 2;
        console.log("Level 2");
        var result = await this.sendCommand("enable", "OLT#");
        console.log(result);
        console.log("Level 3");
        this.Currentlevel = 3;
        var result = await this.sendCommand("config", 'OLT(config)#');
        console.log(result);
    }
    async ShowList() {
        console.log("ShowList");
        if (this.Currentlevel == 3) {
            var res = await this.sendCommand("show ont info all", 'OLT(config)#');
            if (res.status) {
...
function OpenTelnet(options) {
    return new TelnetTplinkUtilty(options)
}

module.exports = OpenTelnet